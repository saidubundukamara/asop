<script lang="ts">
	...
	type Props = {
		comments: Comment[];
		commentEditFlags: { id: string; canEdit: boolean }[];
		canDeleteAnyComment: boolean;
	};

	let { comments, commentEditFlags, canDeleteAnyComment }: Props = $props();
	...
	function canEdit(id: string): boolean {
		return commentEditFlags.find((f) => f.id === id)?.canEdit ?? false;
	}
